class CompanyCashFlowMetrics:
    ...
    def __init__(self, netIncome):
        ...
        self.fcf = None
        self.earnings_quality_gap = None
    ...
    cash_summary = []
    def get_context(self, revenue, industry_avg_fcf, industry_working_capital, industry_operating_cf):
        
        """
        Returns cash-flow context ready for interpretation, ranking, or a decision tree.
        """
        
        earnings_quality_gap = abs(self.operatingCF - self.netIncome)
        ...
        return {
            ...
            "industry_avg_fcf": industry_avg_fcf,

            "industry_avg_operating_cf": industry_operating_cf,

            "industry_avg_working_capital": industry_working_capital,
            ...
            "above_average_operating_cf":
                self.operatingCF > industry_operating_cf,

            "above_average_free_cash_flow":
                self.fcf > industry_avg_fcf,

            "above_average_working_capital":
                self.workingCapital > industry_working_capital,

            # Quality & liquidity signals
            "strong_liquidity": (
                self.workingCapital > 0
                and self.fcf > 0
                and self.workingCapital > industry_working_capital
                and self.fcf > industry_avg_fcf
            ),
            "earnings_quality_strong": (self.netIncome > 0 and earnings_quality_gap / self.netIncome < 0.25),
            "cash_flow_pressure": self.fcf < 0 or self.operatingCF<0
        }
    """
